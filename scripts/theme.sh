...
# The shortlist `tk theme next` cycles through. Any cmux theme works with set.
THEMES=(
  "Catppuccin Mocha"
  "Violite"
  "Vesper"
...
next_theme() {
  ...
  for ((index = 0; index < count; index++)); do
    if same_theme "$current" "${THEMES[$index]}"; then
